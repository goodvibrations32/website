...
:end:
A pre-production development board is on the way to clean up some ground bounce and a bit of switching noise we measure on the breadboard but most of the value lies on software. The hardware bits are all copied, but modified where needed, from Mutable Instruments' designs. The code is build from scratch by me with influence and inspiration from the STM LL libraries and code from other projects such as [[https://miniaud.io/][miniaudio]], [[https://www.dafx.de/DAFX_Book_Page/][DAFX]] but most embedded best moves came from pichenettes. The vendor libraries are used wherever possible to minimize confusion for a new reader.
** documentation
There is a primitive man page for the basic and also the README has the same info twice... There will be a separation when the online resources are comparable and equivalent but for now building instructions are in both places for ease. Duplication of information is always bad but here we are.
** source code structure
:properties:
:custom_id: code
:end:
The latest versions and history is on [[https://github.com/awfulbytes/experiments][github]]. This means that master breaks backwards compatibility during that phase. If there is a major thing, this will be explained in the commit message.  The main idea of what the machine will be capable is there but polishing and final touches aren't.
